...
if [ "$2" == "generic" ]; then
	...
	sed -n '/^# Light sensors$/,/^# end of Light sensors$/ { /^CONFIG/p }' ./kernels/$1/out/.config >> ./kernels/$1/arch/x86/configs/chromeos_defconfig || { echo "Kernel $1 light sensor module configuration failed"; exit 1; }
